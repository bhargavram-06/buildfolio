      ...
      <div className="max-w-6xl mx-auto space-y-12">
        <div className="flex items-center justify-between border-b border-zinc-800 pb-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-mono tracking-wider font-semibold text-[#a1a1aa]">BUILDFOLIO // VERIFIED_PROOF_OF_WORK</span>
          </div>
          <a href={`https://github.com/${username}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs font-medium text-[#a1a1aa] bg-[#16181a] border border-zinc-800 px-3 py-1.5 rounded-lg">
            <FolderGit2 className="w-3.5 h-3.5" /> GitHub Profile
          </a>
        </div>

        <div className="text-right">
          <span className="text-[10px] font-mono bg-zinc-900 border border-zinc-800 text-[#15803d] px-2.5 py-1 rounded-md">Active Theme: {theme || "Silent Coder"}</span>
        </div>
          ...
          <div className="lg:col-span-1 space-y-6 flex flex-col">
            <div className={`rounded-xl p-6 border flex-1 flex flex-col justify-between ${themeStyles.cardStyle}`}>
              <div className="space-y-4">
                ...
                <p className="text-xs text-[#a1a1aa] bg-black/40 p-3 rounded-lg border border-zinc-800/60">{profile.bio}</p>
              </div>
            </div>
